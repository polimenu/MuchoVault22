...
contract btcvaultv2 is ReentrancyGuard, Ownable {
    ...
    address GMarketAddress = 0x47c031236e19d024b42f8AE6780E44A573170703;
    ...
    function getWithdrawWBTCneeded() external view returns (uint256) {
        if (totalPendingWithdraw.ethAmount >= WBTC.balanceOf(address(this))) {
            return totalPendingWithdraw.ethAmount.sub(WBTC.balanceOf(address(this)));
        } else {
            return 0;
        }
    }
    ...
    function getFreeWBTC() external view returns (uint256) {
        if (totalPendingWithdraw.ethAmount >= WBTC.balanceOf(address(this))) {
            return 0;
        } else {
            return WBTC.balanceOf(address(this)).sub(totalPendingWithdraw.ethAmount);
        }
    }
    ...
    function withdraw() public nonReentrant {
        ...
        WBTC.safeTransfer(msg.sender, ethAmountSendOut);
    ...
    function withdrawable(address account) public view returns (bool) {
        ...
        return (WBTC.balanceOf(address(this)) >= ethAmountSendOut && USDC.balanceOf(address(this)) >= usdcAmountSendOut);
    }
    ...
    function _buyGMtokens(
        uint256 wntAmount,
        address tokenReceiver,
        address token1,
        uint256 amount1,
        address token2Receiver,
        address token2,
        uint256 amount2,
        address callback,
